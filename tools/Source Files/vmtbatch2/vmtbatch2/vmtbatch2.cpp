	...
	if ((dir = opendir(name)) != NULL)
	{
		...
		for(int i=1;i<=existent;i++)
		{
			if (namelist[i].size() >= 3) //condition below checks if file is image, diffusse texture
				if ((namelist[i].substr(namelist[i].size() - 3) == "dds" || namelist[i].substr(namelist[i].size() - 3) == "png" || namelist[i].substr(namelist[i].size() - 3) == "jpg" || namelist[i].substr(namelist[i].size() - 3) == "tga") && namelist[i].find(difconvention)!=string::npos && !(namelist[i].find("_spe") != string::npos || namelist[i].find("_sp") != string::npos || namelist[i].find("_nrm") != string::npos || namelist[i].find("_tn") != string::npos) && (namelist[i].at(namelist[i].length() - 5) != 'n' && namelist[i].at(namelist[i].length() - 5) != 's'))
				{
					cout << namelist[i];
